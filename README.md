...
- Destroy Vagrant and close the VirtualBox
- Enable Kubernetes on Docker Desktop
  https://docs.docker.com/desktop/kubernetes/#enable-kubernetes
- Run `skaffold build` which builds all docker images locally if push: false (line 9 in skaffold.yaml).Otherwise, pulls the images from Docker Hub repository (if not exists, creates one and pushed to the repository)
- Run `skaffold dev` which deploys all the yaml files specified in the skaffold.yaml and starts react application on port 30000.
  All other APIs also get starts on specified ports in the Dockerfiles ( Persons API at port 30004, Locations API at port 30003, Connections API at 30002)
...
- In another terminal, start gRPC server by running the following command
  `python3 modules/locationsapi_post/app/locations_grpc_server.py`
- Test gRPC server, by running the following command
